...
*   **Multi-Cloud IaC Generation (Initial Support):**
    *   Based on the parsed intent, the Infrastructure Agent in the backend can generate Terraform HCL for:
        *   **AWS:** EC2 Instances (basic) and S3 Buckets (with versioning, private ACL by default).
        *   **Azure:** Blob Storage Containers (including necessary Storage Account and Resource Group).
        *   **Google Cloud (GCP):** Storage Buckets (with versioning).
    *   The generation logic uses the specified cloud, resource type, resource name, and region (with defaults if not provided via UI/NLP).
...
*   **For GCP (Specific Requirement):**
    *   In addition to standard GCP authentication (e.g., via `GOOGLE_APPLICATION_CREDENTIALS` environment variable pointing to a service account key file), the backend service currently requires the **GCP Project ID** to be explicitly available as an environment variable for generating correct HCL for GCP resources:
        ```bash
        export GCP_PROJECT_ID="your-gcp-project-id"
        ```
    *   Ensure this environment variable is passed to the backend Docker container (e.g., using `-e GCP_PROJECT_ID=$GCP_PROJECT_ID`) if you intend to generate or apply HCL for GCP.

Failure to provide valid credentials (and the `GCP_PROJECT_ID` for GCP) will result in errors when `terraform apply` is executed or HCL is generated by the platform.
...
4.  In the text area provided, describe the application you want to build or the specific cloud resource you need. For example:
    > *"Deploy a Python Flask web application with a PostgreSQL database on AWS. Include SAST scanning."*
    >
    > Or for storage buckets:
    >
    > *   `"Create an AWS S3 bucket named my-unique-photo-archive in us-west-2."`
    > *   `"I need an Azure blob container called 'backupfiles' for my project in the East US region."`
    > *   `"Make a Google Cloud Storage bucket. Call it 'gcp-project-data-store'. Use europe-west1 as the location."`

5.  Optionally, use the "Specify Region/Location" input field below the main text area if you want to explicitly set a region. If a region is mentioned in the main description and also in this optional field, the LLM might prioritize one based on its training; for best results, be consistent or use one method. If no region is specified, sensible defaults for the chosen cloud provider will be used for generation.

6.  Click the "Generate Configuration" (or similarly named) button.
7.  The UI will first display the "Parsed Intent" as a JSON object, showing how the AI understood your request (including any specified or inferred region, resource type, etc.).
8.  Subsequently, it will display the "Generated IaC (Terraform HCL)" based on this parsed intent. You can then validate and apply this configuration using the provided buttons.
